...
describe('#openScratchCard()', () => {
	...
	it('Should return a Promise', () => assert.instanceOf(VxPayJs.openScratchCard(), Promise));
	it('Resets token for tab', (done) => {
		VxPayJs.openScratchCard().then((vxpay) => {
	...
	it('FLow options will be passed to open command', (done) => {
		...
		VxPayJs.openScratchCard(options)
			.then((vxpay) => {
				assert.instanceOf(vxpay, VXPay);
				sandbox.assert.calledOnce(VXPayPromoCode.open);
