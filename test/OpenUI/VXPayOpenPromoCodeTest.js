...
import VXPayPromoCode                        from '../../src/VXPay/Middleware/Command/VXPayPromoCode';
...
describe('#openPromoCode()', () => {
	...
	beforeEach(() => {
		...
		sandbox.stub(VXPayPromoCode, 'open').returns(VxPayJs);
	});

	afterEach(sandbox.restore);

	it('Should return a Promise', () => assert.instanceOf(VxPayJs.openPromoCode(), Promise));
	it('Resets token for tab', (done) => {
		VxPayJs.openPromoCode().then((vxpay) => {
	...
	it('Calls the tab/frame to open login', (done) => {
		VxPayJs.openPromoCode().then((vxpay) => {
			assert.instanceOf(vxpay, VXPay);
			sandbox.assert.calledOnce(VXPayPromoCode.open);
			done();
		});
	});
	it('FLow options will be passed to open command', (done) => {
		...
		VxPayJs.openPromoCode(options)
			.then((vxpay) => {
				assert.instanceOf(vxpay, VXPay);
				sandbox.assert.calledOnce(VXPayPromoCode.open);
				assert.equal(VXPayPromoCode.open.getCall(0).args[1], options, 'Options are not passed');
				done();
			});
	});
});
