...
import VXPayAbo                              from '../../src/VXPay/Middleware/Command/VXPayAbo';
...
describe('#openAbo()', () => {
	...
	beforeEach(() => {
		...
		sandbox.stub(VXPayAbo, 'open').returns(VxPayJs);
	});

	afterEach(sandbox.restore);

	it('Should return a Promise', () => assert.instanceOf(VxPayJs.openAbo(), Promise));
	it('Resets token for tab', (done) => {
		VxPayJs.openAbo().then((vxpay) => {
	...
	it('Calls the tab/frame to open login', (done) => {
		VxPayJs.openAbo().then((vxpay) => {
			assert.instanceOf(vxpay, VXPay);
			sandbox.assert.calledOnce(VXPayAbo.open);
			done();
		});
	});
	it('FLow options will be passed to open command', (done) => {
		...
		VxPayJs.openAbo(options)
			.then((vxpay) => {
				assert.instanceOf(vxpay, VXPay);
				sandbox.assert.calledOnce(VXPayAbo.open);
				assert.equal(VXPayAbo.open.getCall(0).args[1], options, 'Options are not passed');
				done();
			});
	});
});
